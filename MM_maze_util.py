...
# This script was taken from the github repo of Rosenberg et al 2021 (https://github.com/markusmeister/Rosenberg-2021-Repository)
# for plotting some of our figures
...
def PlotMazeWall(m,axes=None,figsize=4):
    ...
    '''
    if axes:
        plot(m.wa[:,0],m.wa[:,1],fmts=['k-'],equal=True,linewidth=1.0,yflip=True,
...
def PlotMazeNums(m,ax,mode='cells',numcol='blue'):
    ...
    if mode=='nodes':
        for j,r in enumerate(m.ru):
            ...
            plt.text(x-.35,y+.15,'{:d}'.format(j),color=numcol, fontsize=2) # number the ends of a run    
    if mode=='cells':
        ...
        for j,r in enumerate(m.ru):
            ...
            if not(f is None):
                ...
                ax.add_patch(patches.Rectangle((x-0.5,y-0.5),1.0,1.0,lw=0,
                                           fc=cmap(norm(f[j])), ec='gray')) # draw with color f[]
            if numcol:
                plt.text(x-.35,y+.15,'{:d}'.format(j),color=numcol) # number the ends of a run   
            plt.text(x-0.35,y+.15,'.',color='black',  fontsize=10) # put dots on the maps
